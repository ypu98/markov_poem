...
def activity_forecast(days, init):
    ...
    if init == st3:
        activityToday = np.random.choice(st3)
    else:
...
minLen = 6
maxLen = 10
for iter in range(1, numSen/2+1):
    list_activity.append(activity_forecast(np.random.randint(minLen,maxLen+1), st3))
    list_activity.append(activity_forecast(np.random.randint(minLen,maxLen+1), st1))
